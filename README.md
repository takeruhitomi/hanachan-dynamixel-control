...
| 10 | Turn | 左右の駆動差。`0.5` で軸旋回、`±1.0` で**その場回転** |
| 11 | Segment phase | 前後セグメント間の脚位相差 |
| 12 | Left/right phase | 左脚群と右脚群の位相差 |
| 13 | Body wave | 胴体関節ID 1〜3の周期的な曲げ振幅。上限で片振幅 22.5°（可動域 ±25°） |
...
`Turn` は左右の駆動量を差動で振り分けます。片側を弱めるだけでは、止まった側を軸にした旋回にしかなりません。その場で回るには片側が逆向きに蹴る必要があるため、駆動量は負まで振れるようにしてあります。
...
反転するのは前後のストロークだけです。`Foot clearance` はどちら向きに振れていても足を持ち上げます。
...
PC にサーバーを立て、スマホのブラウザから操作します。PC は持ち歩く必要はありません。無線がロボットに届く場所に置いたままで構いません。

```powershell
uv run python main.py --device COM7 --baudrate 57600 serve
```

起動すると接続先が表示されます。

```
open on your phone:  http://100.122.128.83:8080/   (tailnet)
                     http://192.168.0.21:8080/     (local network)
```

大学のWiFiのようにクライアント間通信が遮断される環境では、tailnet 側のアドレスを使ってください。スマホにも Tailscale が入っていれば、そのままブラウザで開けます。アプリのインストールは不要です。
...
| 初期姿勢 | 2秒かけてニュートラル姿勢へ |
| 停止 | 動作を止めてニュートラルへ |
| 歩容 / モーション | `gait_presets.json` の歩容と、`teach` で記録したモーションを切り替えて選択 |
| 走行 / 再生 | 選んだものを実行。歩容は停止するまで歩き続け、モーションは1回だけ再生して止まります |
| Frequency | 選択中の動きの速さ |
| Turn / Body turn / Body wave | 歩容選択時のみ。旋回と胴体の動き |

歩容の細かい調整は `cpg-gui-v2` で行い、プリセットとして保存しておく想定です。スマホ側は選んで走らせるためのものなので、全パラメータは出していません。

モーションは `motions.json` から読み込まれます（`--motion-file` で変更可）。`teach` で名前を付けて保存したものがそのまま選択肢になります。モーション選択時は再生速度の倍率だけを調整でき、旋回や胴体のパラメータは出ません。

`Turn` と `Body turn` は中央に**デッドゾーン（±0.06）**があります。スマホのスライダーで正確に0を出すのは難しく、わずかにずれたまま走らせると機体が曲がっていってしまうためです。この範囲内は直進として扱われ、スライダー自体も指の下で0に吸着します。ラベル横の `0` ボタンでも中央に戻せます。

**選択しただけでは動きません。** 一覧をスクロールした拍子に歩き出さないよう、実行は `走行` / `再生` ボタンを押したときだけです。モーションは最後まで再生すると自動で止まり、状態が `finished` になります。

画面下部にリンク状態（制御周期、tick数、通信エラー数）が出ます。無線が不安定になったときに、原因が通信なのか設定なのかを切り分けられます。**通信エラーが起きても動作は止まりません。** 連続25回失敗した場合のみ停止して `link lost` を表示します。

`--host` は既定で全インターフェースを待ち受けます。tailnet だけに限定したい場合は `--host 100.x.x.x` のように指定してください。認証はないので、共有ネットワークで開けたままにしないでください。

## モーションの記録と再生

歩容ではない動き（キック、踊りなど）はここで作ります。トルクを切って手で姿勢を作り、**全サーボの現在位置**をフレームとして取り込みます。胴体関節も含まれます。

`cpg-gui` の **`Motion Teaching...`** ボタンから開けます。シリアルポートは排他なので、この場合は歩容GUIのバスをそのまま共有します。開くと歩容の実行は止まり、全関節が脱力します。

単独で起動することもできます。

```sh
uv run python main.py --device COM6 teach
```

教示GUIは起動時に対象関節のトルクをOFFにします。再生時は実機が動くため、必ず機体を支持した状態で確認してください。

1. 手で姿勢を作り、`現在姿勢を追加` で取り込む。名前と移動時間（前の姿勢からその姿勢までの時間）を指定します
2. 姿勢を並べ替え・削除・取り直しして一連の動きにする
3. `繰り返し再生` にチェックを入れると、最後のフレームから最初へ戻ってループします
4. `モーション一覧` の名前欄に名前を入れて `保存`

複数のモーションが1つの `motions.json` に名前付きで入ります。`読込` で切り替え、`削除` で消せます。**この名前がそのままスマホ側の選択肢になります。**

```json
{
  "format": "hanachan-motion-library",
  "motions": [
    { "name": "kick", "servo_ids": [...], "frames": [...] },
    { "name": "wave", "servo_ids": [...], "frames": [...] }
  ]
}
```

保存先は `--output` で変更できます（既定 `motions.json`）。

### 旧形式のファイル

名前を持たない旧形式（1ファイル1モーション）もそのまま読めます。ファイル名が名前になります。

- `motions.json` がまだ無い場合、隣にある `motion_sequence.json` を自動で読み込みます。教示GUIの一覧にもスマホ側にも `motion_sequence` として出ます
- `ファイルから読込...` からも開けます。読み込むと名前欄に候補が入るので、付け直して `保存` すれば `motions.json` に移せます
- サーバーに直接指定することもできます: `serve --motion-file motion_sequence.json`

歩容の設計は `cpg-gui-v2` の担当です。こちらは姿勢の記録・再生に専念します。
