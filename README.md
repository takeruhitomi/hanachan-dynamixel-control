...
`--baudrate` が 115200 以下のとき、リトライ回数の増加とパケットタイムアウトの延長が自動で有効になります。制御周期は既定で 30 Hz です（有線は 50 Hz）。`--control-hz` で変更できます。

### 帯域の目安

57600 bps では毎秒 5760 バイトしか運べません。21関節への sync write が1回あたり約119バイトなので、実測は次のようになります。

| 制御周期 | 帯域使用率 | 実測 |
|---|---|---|
| 30 Hz | 62% | 15秒間でオーバーラン 0.45%、書き込み失敗 0 |
| 40 Hz | 83% | 12秒間でオーバーラン 0、書き込み失敗 0 |
| 45 Hz | 93% | 書き込みが帯域を占有し、読み取りが応答なしになる |

既定の 30 Hz は余裕を見た値です。書き込みだけなら 40 Hz まで実用になりますが、位置の読み取りを併用すると破綻します。

読み取りは無線では高価です（21関節の全読み取りで約 800 ms）。CPG は毎周期では読まず初期化時のみなので、歩容の実行には影響しません。
